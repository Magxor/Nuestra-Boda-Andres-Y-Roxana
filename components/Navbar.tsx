    ...
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ${scrolled ? 'bg-white/95 backdrop-blur-md shadow-md py-3' : 'bg-transparent py-4 sm:py-6'}`}>
      <div className="container mx-auto px-6 flex justify-between items-center">
        ...
        <div className="flex items-center relative">
            ...
            className={`relative z-10 px-5 sm:px-7 py-2.5 rounded-full text-xs sm:text-sm font-medium tracking-widest uppercase transition-all duration-300 flex flex-col items-center leading-tight shadow-lg ${
            scrolled
           ...
           >
              ...
              <span className={`text-[9px] sm:text-[10px] font-sans normal-case tracking-wide opacity-90 ${scrolled ? 'text-blue-40' : 'text-slate-500'}`}>antes del 05/01/2026</span>
            </button>
        </div>
      </div>
    </nav>
